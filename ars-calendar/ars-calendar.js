...
import {createPieGraphWithEvenlyDistributedColors} from '../../arslib/util/image.js'
...
class ArsCalendar extends WebComponentBase {
  constructor() {
    ...
    this.localizedAbbreviatedDays = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat',]
    ...
    this.addEvent =  function(event) {
      const sameEventFound = this.events.find(ev => EObject.hasSameProperties(ev, event))
    ...
    this.removeEvent = function (eventDate) {
      ...
      this.events = this.events.filter(
        ev =>
          ev.text !== eventDate.text || ev.day !== eventDate.day || ev.month !== eventDate.month || ev.year !== eventDate.year
      )
    ...
    this.changeEvent = function (eventDate, newText, newColor) {
      const event = this.events.find(
        ev =>
          ev.day === eventDate.day && ev.month === eventDate.month && ev.year === eventDate.year
      )
    ...
    // Swipe support
    this.onSwipeRight = function()  { this.previousMonth()  }
    this.onSwipeLeft  = function()  { this.nextMonth()      }
  ...
  _getEventsByDate(day,month,year) {
    return this.events.filter(event => event.day === day && event.month === month && event.year === year)
  }
  ...
  _sendDaySelectedEvent() {
    this.dispatchEvent(new CustomEvent('ars-calendar:daySelected', {
      detail: {
        'id': this.id,
        'day': this.selectedDay,
        'month': this.selectedMonth,
        'year': this.selectedYear,
        'events': this._getEventsByDate(this.selectedDay, this.selectedMonth, this.selectedYear)
      },
  ...
  _render() {
    ...
    let dayElements = this.shadowRoot.querySelectorAll('.curr > tbody > tr > td')
    for(let weekIndex=0; weekIndex < this.WEEKS_IN_MONTH; weekIndex++) {
      for(let dayOfWeekIndex=0; dayOfWeekIndex < this.DAYS_IN_WEEK; dayOfWeekIndex++) {
        let dayElement = dayElements[this.getDaySlotIndex(weekIndex, dayOfWeekIndex)]
        let backgroundCanvas = this.daySlotsColors[this.getDaySlotIndex(weekIndex, dayOfWeekIndex)]
        ...
        dayElement.innerText = this.daySlots[this.getDaySlotIndex(weekIndex, dayOfWeekIndex)]
        dayElement.onclick = () => { 
          eval(`this.onDayClicked(this.getDaySlotIndex(${weekIndex}, ${dayOfWeekIndex}))`)
        }

        //change class if selected day
        if(
          this.selectedDay === this.daySlots[this.getDaySlotIndex(weekIndex, dayOfWeekIndex)] &&
  ...
  static get observedAttributes() {
    return ['localized_abbreviated_days', 'localized_months', 'localized_today']
  }
  ...
  fillDaySlots(month, year) {
    ...
    let dayNumber = 1
    for (let daySlotIndex = firstDay; daySlotIndex < numDays + firstDay; daySlotIndex++) {
      this.daySlots[daySlotIndex] = dayNumber
      this.daySlotsColors[daySlotIndex] =
        this.getColorCanvasFromDate(dayNumber, month, year)
      dayNumber++
    }
  }
  ...
  getColorCanvasFromDate(day, month, year) {
    ...
    if (!colorsForDate.length) {
      return createPieGraphWithEvenlyDistributedColors(cellWidth, cellHeight, ['white']) 
    } 
    return createPieGraphWithEvenlyDistributedColors(cellWidth, cellHeight, colorsForDate)
  }

  buildDayAndClassSlots() {
    this.daySlots = new Array(this.WEEKS_IN_MONTH * this.DAYS_IN_WEEK).fill(null)
    this.daySlotsColors = this.daySlots.map(() => createPieGraphWithEvenlyDistributedColors(cellWidth, cellHeight, ['white']))
  }
