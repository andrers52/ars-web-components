...
class ArsDataRoller extends WebComponentBase {
  ...
  #animateRoll() {
    ...
    (current as HTMLElement).style.transition = 'none';
    (current as HTMLElement).style.transform = 'translateY(0%) rotateX(0deg)';
    (current as HTMLElement).style.opacity = '1';
    (next as HTMLElement).style.transition = 'none';
    (next as HTMLElement).style.transform = 'translateY(60%) rotateX(-90deg)';
    (next as HTMLElement).style.opacity = '0';
    (next as HTMLElement).style.zIndex = '2';
    (next as HTMLElement).style.display = 'block';

    // Animate both
    setTimeout(() => {
      (current as HTMLElement).style.transition = `transform ${this.animationDuration}ms cubic-bezier(0.4,0.2,0.2,1), opacity ${this.animationDuration}ms`;
      (next as HTMLElement).style.transition = `transform ${this.animationDuration}ms cubic-bezier(0.4,0.2,0.2,1), opacity ${this.animationDuration}ms`;
      (current as HTMLElement).style.transform = 'translateY(-60%) rotateX(90deg)';
      (current as HTMLElement).style.opacity = '0';
      (next as HTMLElement).style.transform = 'translateY(0%) rotateX(0deg)';
      (next as HTMLElement).style.opacity = '1';
      ...
      setTimeout(() => {
        ...
        if (newCurrent && newNext) {
          ...
          (newCurrent as HTMLElement).style.opacity = '1';
          (newNext as HTMLElement).style.transition = 'none';
          (newNext as HTMLElement).style.transform = 'translateY(60%) rotateX(-90deg)';
          (newNext as HTMLElement).style.opacity = '0';
          (newNext as HTMLElement).style.zIndex = '2';
          (newNext as HTMLElement).style.display = 'block';
        }
