...
class ShowIfPropertyTrueMixin extends WebComponentBase {
  ...
  #updateVisibility() {
    ...
    if (shouldShow) {
      (targetComponent as HTMLElement).style.display = this._originalDisplay || "block";
      (targetComponent as HTMLElement).style.visibility = "";
    } else if (keepSpace) {
      (targetComponent as HTMLElement).style.display = this._originalDisplay || "block";
      (targetComponent as HTMLElement).style.visibility = "hidden";
    } else {
      (targetComponent as HTMLElement).style.display = "none";
      (targetComponent as HTMLElement).style.visibility = "";
    }
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === "show-if-property") {
      if (this.#validateProperty(newValue)) {
        ...
        const targetComponent = this.firstElementChild;
        if (targetComponent) {
          (targetComponent as HTMLElement).style.display = this._originalDisplay || "";
        }
      }
    }
  }
